...
function App() {
  const [localidade, setLocalidade] = React.useState([null, null]);
  const [tempos, setTempos] = React.useState(null);

  React.useEffect(() => {
    fazerFetch("https://extreme-ip-lookup.com/json/").then((dadosIp) => {
      const { lat, lon, city, region } = dadosIp;
      setLocalidade([city, region]);
      fazerFetch(
        `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=metric&lang=pt_BR&appid=47b16d41765388e1d2e251b373b570c0`
      ).then((dadosTempo) => {
        setTempos(dadosTempo.daily);
      });
    });
  });
  ...
  return (
    <div className="App">
      <h1>
        ...
        {localidade[0] === null ? "Carregando" : localidade[0]}
        {localidade[1] === null ? "" : `, ${localidade[1]}`}
      </h1>
      <ul>
        {tempos &&
          tempos.map((tempo, i) => (
            <li>
              ...
              ></img>
              <h2>
                {i === 0 ? "Hoje" : i === 1 ? "Amanhã" : `Daqui a ${i} dias`}
              </h2>
              <div className="descricao">
                <span>{tempo.weather[0].description}</span>
                <span>
                  {tempo.temp.min}°C - {tempo.temp.max}°C 
                </span>
              </div>
            </li>
          ))}
